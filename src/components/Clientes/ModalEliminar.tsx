...
function ModalEliminar({ isOpen, closeModal, clients, handleRemoveSubmit }: ModalEliminarProps) {
      ...
      >
        <Box sx={styleModal}>
          <Box className="headerModal" px={2} py={1} borderRadius={"10px 10px 0px 0px"} bgcolor={theme.palette.error.dark} color="theme.palette.success.contrastText">
            <input type="button" value="test" onClick={() => console.log(clients)} />
          ...
          <Box className="bodyModal" px={2} py={2} sx={{ display: 'flex', flexDirection: 'column' }} borderRadius={"0px 0px 10px 10px"}>
            <Typography variant="h6" textAlign="start" component="h3">{clients.length > 1 ? "Vas a los siguientes usuarios: " : "Vas a eliminar el siguiente usuario:"}</Typography>
            <List>
            ...
            <Box sx={{ display: 'flex', gap: 5, justifyContent: 'flex-end' }}>
              ...
              <Button variant="contained" color="success" onClick={() => handleRemoveSubmit(clients)}>
                Aceptar
              </Button>
            </Box>
          </Box>
